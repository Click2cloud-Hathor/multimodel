...
if ($null -ne $hasOwnerRole) {
    ...
    Write-Host "User does not have Owner permission on the subscription. Deployment will fail. Would you still like to continue? Press Y/N" -ForegroundColor Red
...
$useOpenAI = Read-Host "by default Azure Databricks model will be used for text embedding ,do you want to use the OpenAI endpoint for text embedding? (yes/no)"
...
if ($deploymentStatus -eq "Succeeded") {
    Write-Host "Template deployment succeeded. Have you provided yourself as account administrator on Databricks? Press Y/N"
...
$secret = az keyvault secret set --vault-name $keyVaultName --name "databricks-token" --value $pat_token 
